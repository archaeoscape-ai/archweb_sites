...
//print "<!-- updating because " . $child->name . " has changed -->\n";
		...
		foreach ($this->children as $child) {
			if (is_a($child, 'Auto'))
				...
				$value = $child->value($vars);
				//if (! $value  and  $child->required) return false;
				if (($value === ""  or  $value === NULL)  and  $child->required) return false;  // --kj, 2008-02-16
				$updates[$child->name] = $value;
			}
		}
...
//print "processing " . $this->component_description() . "<br>";
			if (is_a($child, 'Auto'))
				...
				if (($value === ""  or $value === NULL)  and  $child->required)	// --kj, 2008-02-16
					return true;
				if ($value != NULL) $inserts[$child->name] = $value;
			}
...
//				foreach ($cols as $key => $value) $vars[$key] = $vars[$this->name . ".$key"] = $value;
		...
		return array($my_errors, $my_sats);
	}
...
class Datebox extends Textbox {
	...
	function parseDate($value) {
		...
		if (preg_match('/\\b[12]\\d{3}\\b/', $value, $matches)) {
			/* recognise four-digit years ... pull them out and replace them with the year 1999 so strtotime can handle it */
			$year = $matches[0];
			if (trim($value) == $year) {
				// just the year was specified
				return "$year-01-01";
			} else {
				return date("$year-m-d", strtotime(preg_replace("/\\b$year\\b/", "1999", $value)));
			}
		}
